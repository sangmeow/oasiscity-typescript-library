 ...
 */
export const isValidProbabilityInput = (probability: number, outOf: number): boolean => {
  // Check for valid numbers
  if (!Number.isFinite(probability) || !Number.isFinite(outOf)) { return false; }
  // Check for positive values
  if (probability < 0 || outOf <= 0) { return false; }
  // Probability cannot be greater than total outcomes
  if (probability > outOf) { return false; }
  return true;
};
 ...
 */
export const lottery = (probability: number = 1, outOf: number = 100): boolean => {
  // Input validation
  if (!isValidProbabilityInput(probability, outOf)) { return false; }
